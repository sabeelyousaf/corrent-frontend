...
const NavDropDown = ({ user }) => {
    ...
    return (
        <div ref={dropdownRef} className='border p-2 border-zinc-200 rounded-md flex items-center gap-2 relative'>
           <img
...
/>
            ...
            >
                <ul>
                    <li className='border-b border-zinc-300 p-3 flex items-center gap-2'>
                        <img className='w-12 h-12 rounded-full object-cover object-center' src={user?.avatar?.secure_url} alt=""  onError={(e) => e.target.src = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRhtMRbtowke9ZnnGtyYJmIuJaB2Q1y5I-3IA&s"} />
                        <div>
    ...
    {[
        ...
        // { label: "Favorites", icon: FaRegHeart, path: "" },
    ].map((r, index) =>
        <Link
            key={index}
